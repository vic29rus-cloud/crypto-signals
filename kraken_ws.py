...
1. Исключены FIAT и стейблы.
2. Пробой только по закрытой свече + буфер 1.5%.
3. Защита от перегретого тренда (ADX max 45).
4. Time Stop (закрытие по времени).
5. Раздельное отображение позиций Confluence и Breakout.
"""
